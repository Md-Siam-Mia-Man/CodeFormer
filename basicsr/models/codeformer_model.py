...
class CodeFormerModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if (
            self.opt.get("network_vqgan", None) is not None
            and self.opt["datasets"].get("latent_gt_path") is None
        ):
        ...
        if self.fidelity_weight > 0:
            ...
            if load_path is not None:
                self.load_network(
                    self.net_d, load_path, self.opt["path"].get("strict_load_d", True)
                )
    ...
    def calculate_adaptive_weight(
        self, recon_loss, g_loss, last_layer, disc_weight_max
    ):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.fidelity_weight > 0:
            self.output, logits, lq_feat = self.net_g(
                self.input, w=self.fidelity_weight, detach_16=True
            )
        else:
        ...
        if self.hq_feat_loss:
            ...
            quant_feat_gt = self.net_g.module.quantize.get_codebook_feat(
                self.idx_gt, shape=[self.b, 16, 16, 256]
            )
        ...
        if (
            ...
            and current_iter > self.net_g_start_iter
        ):
            # hq_feat_loss
            if self.hq_feat_loss:  # codebook loss
                l_feat_encoder = (
                    torch.mean((quant_feat_gt.detach() - lq_feat) ** 2)
            ...
            if self.cross_entropy_loss:
                # b(hw)n -> bn(hw)
                cross_entropy_loss = (
                    F.cross_entropy(logits.permute(0, 2, 1), self.idx_gt)
            ...
            if (
                self.fidelity_weight > 0
            ):  # when fidelity_weight == 0 don't need image-level loss
                # pixel loss
                if self.cri_pix:
                ...
                if current_iter > self.net_d_start_iter:
                    ...
                    if not self.fix_generator:
                        ...
                        d_weight = self.calculate_adaptive_weight(
                            recon_loss, l_g_gan, last_layer, disc_weight_max=1.0
                        )
                    else:
                        largest_fuse_size = self.opt["network_g"]["connect_list"][-1]
                        last_layer = (
                            self.net_g.module.fuse_convs_dict[largest_fuse_size]
                            .shift[-1]
                            .weight
                        )
                        d_weight = self.calculate_adaptive_weight(
