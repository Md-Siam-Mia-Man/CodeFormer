...
class FFHQBlindJointDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_gt, status = augment(
            img_gt, hflip=self.opt["use_hflip"], rotation=False, return_status=True
        )
        ...
        if self.use_corrupt:
            ...
            scale = np.random.uniform(
                self.downsample_range[0], self.downsample_range[1]
            )
            img_in = cv2.resize(
                img_in,
                (int(self.gt_size // scale), int(self.gt_size // scale)),
            ...
            if self.noise_range is not None:
                noise_sigma = np.random.uniform(
                    self.noise_range[0] / 255.0, self.noise_range[1] / 255.0
                )
            ...
            if self.jpeg_range is not None:
                ...
                _, encimg = cv2.imencode(".jpg", img_in * 255.0, encode_param)
                img_in = np.float32(cv2.imdecode(encimg, 1)) / 255.0

            # resize to in_size
            img_in = cv2.resize(
                img_in, (self.in_size, self.in_size), interpolation=cv2.INTER_LINEAR
            )
        ...
        if self.use_corrupt:
            ...
            # downsample
            scale = np.random.uniform(
                self.downsample_range_large[0], self.downsample_range_large[1]
            )
            img_in_large = cv2.resize(
                img_in_large,
                (int(self.gt_size // scale), int(self.gt_size // scale)),
                interpolation=cv2.INTER_LINEAR,
            )
            ...
            if self.noise_range_large is not None:
                noise_sigma = np.random.uniform(
                    self.noise_range_large[0] / 255.0, self.noise_range_large[1] / 255.0
                )
            ...
            if self.jpeg_range_large is not None:
                jpeg_p = np.random.uniform(
                    self.jpeg_range_large[0], self.jpeg_range_large[1]
                )
                encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), int(jpeg_p)]
                _, encimg = cv2.imencode(".jpg", img_in_large * 255.0, encode_param)
                img_in_large = np.float32(cv2.imdecode(encimg, 1)) / 255.0
        ...
        if self.color_jitter_prob is not None and (
            np.random.uniform() < self.color_jitter_prob
        ):
        ...
        img_in, img_in_large, img_gt = img2tensor(
            [img_in, img_in_large, img_gt], bgr2rgb=True, float32=True
        )
        ...
        if self.color_jitter_pt_prob is not None and (
            ...
            img_in_large = self.color_jitter_pt(
                img_in_large, brightness, contrast, saturation, hue
            )

        # round and clip
        img_in = np.clip((img_in * 255.0).round(), 0, 255) / 255.0
        img_in_large = np.clip((img_in_large * 255.0).round(), 0, 255) / 255.0
